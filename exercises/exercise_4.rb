...
@mens_stores.each do |store|
  puts "Store name: #{store.name}, Annual Revenue: #{store.annual_revenue}, #{store.mens_apparel}"
end

# Do another fetch but this time load stores that carry women's apparel
#  and are generating less than $1M in annual revenue.
@women_stores_less_revenue = Store.where(womens_apparel: true).where(annual_revenue: 0...1_000_000)

@women_stores_less_revenue.each do |store|
  puts "Store name: #{store.name}, Annual Revenue: #{store.annual_revenue}, #{store.womens_apparel}"
end
